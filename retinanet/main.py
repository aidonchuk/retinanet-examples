...
def parse(args):
    ...
    parser.add_argument('--master', metavar='address:port', type=str, help='Adress and port of the master worker',
    ...
    parser_train.add_argument('--milestones', action='store', type=int, nargs='*',
                              help='list of iteration indices where learning rate decays', default=[15000, 30000])
    ...
    parser_train.add_argument('--val-iters', metavar='number', type=int,
                              help='number of iterations between each validation', default=20)
    ...
    parser_train.add_argument('--model-threshold', type=float, default=0.05)
    parser_train.add_argument('--model-top-n', type=int, default=1000)
    parser_train.add_argument('--model-nms', type=float, default=0.5)
    parser_train.add_argument('--model-detections', type=int, default=100)
...
def load_model(args, verbose=False):
    ...
    if args.command == 'train' and (not os.path.exists(args.model) or args.override):
        ...
        model = Model(args.backbone, args.classes,
                      config={'threshold': args.model_threshold, 'top_n': args.model_top_n, 'nms': args.model_nms,
                              'detections': args.model_detections})
